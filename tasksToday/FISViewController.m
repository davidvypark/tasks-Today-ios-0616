...
-(void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    ...
    FISEventDetailViewController *destinationVC = [segue destinationViewController];
    
    if ([segue.identifier isEqualToString:@"lastYear"]) {
        destinationVC.inputArray = self.lastYearArray;
        
    } else if ([segue.identifier isEqualToString:@"nextYear"]) {
        destinationVC.inputArray = self.nextYearArray;
...
- (IBAction)nextYearButtonPressed:(id)sender {
    ...
    self.nextYearArray = [self getNextYearsEvents];
}

- (IBAction)lastYearButtonPressed:(id)sender {
    
    self.lastYearArray = [self getLastYearsEvents];
}
